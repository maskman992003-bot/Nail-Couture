...
export function TheVault({ points, milestones = [], onMilestonePress }: TheVaultProps) {
  ...
  const mergedMilestones = VAULT_MILESTONES.map((m) => {
        ...
        <View style={{ width: VIAL_WIDTH, height: VIAL_HEIGHT, position: 'relative' }}>
          ...
          {mergedMilestones.map((m) => {
            ...
            return (
              <Pressable
                key={m.points}
                onPress={() => m.unlocked && onMilestonePress?.(m.points)}
                style={{
                  position: 'absolute',
                  top: y - 10,
                  left: VIAL_WIDTH + 4,
              ...
              >
                <View
                  style={{
                    width: glowing ? 14 : 8,
                    height: glowing ? 14 : 8,
                    borderRadius: glowing ? 4 : 4,
                    backgroundColor: m.redeemed ? '#22c55e' : glowing ? styles.tokens.goldStrong : `${styles.tokens.textSecondary}44`,
