...
import { getAppointmentServiceGroups, getAppointmentServices } from '@nail-couture/shared/utils/appointmentHelpers';
...
export default function AppointmentServicesPanel({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const built = await loadVisitServiceSummary(appointment);
  ...
  if (loading && !summary) {
    ...
    const hasImmediateData = groups.main.length > 0 || groups.addons.length > 0;

    if (hasImmediateData) {
      return (
        <div className={clsx('space-y-3', className)}>
          <ServiceLineList label="Services" items={groups.main.map((name) => ({ name, price: null }))} styles={styles} />
          {groups.addons.length > 0 && (
            <ServiceLineList label="Add-ons" items={groups.addons.map((name) => ({ name, price: null }))} variant="addon" styles={styles} />
          )}
          <div className={clsx('text-[10px] animate-pulse', styles.mutedClass)}>Loading history…</div>
        </div>
      );
    }
