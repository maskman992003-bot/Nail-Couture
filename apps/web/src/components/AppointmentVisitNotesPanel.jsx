...
import {
  ...
  getImmediateVisitNotes,
} from '@nail-couture/shared/utils/appointmentVisitNotes';
...
export default function AppointmentVisitNotesPanel({
  ...
  const displayEntries = entries.length > 0 ? entries : (loading ? getImmediateVisitNotes(appointment) : []);

  if (loading && !displayEntries.length) {
    return (
  ...
  return (
    <div className={clsx('space-y-3', className)}>
      ...
      {loading && (
        <div className={clsx('text-[10px] animate-pulse', styles.mutedClass)}>Loading more notes…</div>
      )}
    </div>
  );
}
