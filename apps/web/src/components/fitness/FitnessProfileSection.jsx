...
export default function FitnessProfileSection({ profileId, callerPhone, theme, panelClass }) {
  ...
  if (!available) {
    return (
      <p className={theme === 'dark' ? 'text-offwhite/50 text-sm' : 'text-charcoal/50 text-sm'}>
        Fitness tracking is not enabled on the database yet.
      </p>
    );
  }
  ...
  if (!latest) {
    return (
      <div className={panelClass}>
        ...
        <Link to="/customer/fitness-assessment" className={clsx(modalBtnPrimary, 'inline-flex px-6 max-w-xs')}>
          Start assessment
        </Link>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className={panelClass}>
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          ...
          <Link
            to="/customer/fitness-assessment"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'BMI', value: m.bmi, suffix: m.bmi != null ? ' kg/m²' : '' },
            { label: 'Body fat', value: m.bodyFatPercent, suffix: m.bodyFatPercent != null ? '%' : '' },
            { label: 'BMR', value: m.bmr, suffix: m.bmr != null ? ' kcal' : '' },
            { label: 'TDEE', value: m.tdee, suffix: m.tdee != null ? ' kcal' : '' },
          ].map((item) => (
            <div key={item.label} className={cardClass(theme)}>
              <span className={labelClass(theme)}>{item.label}</span>
              <span className={valueClass(theme)}>
                {item.value ?? '—'}
        ...
        {m.calorieTargets && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
            {[
              { label: 'Weight loss', value: m.calorieTargets.weightLoss },
              { label: 'Maintenance', value: m.calorieTargets.maintenance },
              { label: 'Muscle gain', value: m.calorieTargets.muscleGain },
            ].map((item) => (
              <div key={item.label} className={cardClass(theme)}>
                <span className={labelClass(theme)}>{item.label}</span>
                <span className={valueClass(theme)}>{item.value ?? '—'} kcal/day</span>
              </div>
            ))}
          </div>
        )}
