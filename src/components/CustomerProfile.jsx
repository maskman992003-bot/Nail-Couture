...
export default function CustomerProfile() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (data) {
        setProfile(data);
        setForm({ full_name: data.full_name || '', email: data.email || '', phone_number: data.phone_number || '', nail_goal: data.nail_goal || '', refreshment_pref: data.refreshment_pref || '' });
      }
    } catch (err) {
            ...
            {editing ? (
              <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => { setEditing(false); setForm({ full_name: profile.full_name, email: profile.email, phone_number: profile.phone_number, nail_goal: profile.nail_goal, refreshment_pref: profile.refreshment_pref }); }} className="px-6 py-3 border text-offwhite/60 text-sm rounded-xl hover:border-gold/30" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
                    Cancel
                  </button>
                </div>
              </div>
