-- Migration 078: Per-promotion slide-in auto-hide duration
-- Run once in Supabase SQL Editor after 077_delete_promotion.sql

ALTER TABLE promotions
  ADD COLUMN IF NOT EXISTS slide_in_auto_hide_seconds int NULL;

COMMENT ON COLUMN promotions.slide_in_auto_hide_seconds IS
  'Seconds until compact slide-in chip auto-hides (NULL or 0 = stay until user taps).';

-- ---------------------------------------------------------------------------
-- upsert_promotion — add slide_in_auto_hide_seconds
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  ELSE
    INSERT INTO promotions (
      ...
      slide_in_auto_hide_seconds, created_by
    ) VALUES (
      trim(p_slug),
      p_kind,
      trim(p_title),
