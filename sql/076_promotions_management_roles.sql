-- Migration 076: Allow partner role to manage promotions (same as announcements)
-- Run once in Supabase SQL Editor after 075_promotions.sql

CREATE OR REPLACE FUNCTION promotion_is_management_role(p_role text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
...
COMMENT ON FUNCTION promotion_is_management_role IS
  'Management roles that can CRUD home-screen promotions (matches announcements).';

-- Replace executive-only checks with management role checks
