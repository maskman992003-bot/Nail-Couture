-- Migration 079: Per-promotion local dismiss/copy suppression
-- Run once in Supabase SQL Editor after 078_promotion_slide_in_auto_hide.sql

ALTER TABLE promotions
  ADD COLUMN IF NOT EXISTS suppress_after_dismiss boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS suppress_after_copy boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN promotions.suppress_after_dismiss IS
  'When true, closing the offer detail permanently hides it on this device (localStorage).';

COMMENT ON COLUMN promotions.suppress_after_copy IS
  'When true, copying the promo code permanently hides it on this device (localStorage).';

-- ---------------------------------------------------------------------------
-- upsert_promotion — add suppress_after_dismiss / suppress_after_copy
-- ---------------------------------------------------------------------------
