  ...
  ELSE
    IF order_asc THEN
      SELECT jsonb_agg(sub.* ORDER BY sub.checked_in_at ASC) INTO result FROM (
        SELECT a.*, row_to_json(srv.*)::jsonb AS services, row_to_json(cust.*)::jsonb AS customer, row_to_json(tech.*)::jsonb AS technician
        FROM appointments a
...
BEGIN
  ...
  ELSE
    IF order_asc THEN
      SELECT jsonb_agg(sub.* ORDER BY sub.checked_in_at ASC, sub.scheduled_at ASC) INTO result FROM (
        SELECT a.*, row_to_json(srv.*)::jsonb AS services, row_to_json(tech.*)::jsonb AS technician
        FROM appointments a
        LEFT JOIN services srv ON srv.id = a.service_id
        LEFT JOIN profiles tech ON tech.id = a.technician_id
        WHERE a.customer_id = customer_id
...
-- ============================================================
-- STAFF COUNT RPC: convenience wrapper returning integer
-- ============================================================
...
BEGIN
  ...
  SELECT COUNT(*)::int INTO result FROM appointments a
  WHERE (status_filter IS NULL OR a.status = ANY(string_to_array(status_filter, ',')))
    AND (date_from IS NULL OR a.checked_in_at >= date_from)
    AND (date_to IS NULL OR a.checked_in_at < date_to);
