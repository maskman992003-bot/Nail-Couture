-- Migration 116: Gift card checkout — select from customer cards + 3-char confirmation
-- Run once in Supabase SQL Editor after 115_loyalty_milestone_rewards.sql

-- ---------------------------------------------------------------------------
-- 0) Caller lookup helper (phone formats may differ)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 1) Mask + confirmation helpers
-- ---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION gift_card_checkout_mask(p_code text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_code IS NULL OR trim(p_code) = '' THEN ''
    ELSE 'GC-****-' || right(gift_card_normalize_code(p_code), 4)
  END;
$$;

CREATE OR REPLACE FUNCTION gift_card_checkout_confirm_chars(p_code text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_code IS NULL OR trim(p_code) = '' THEN ''
    ELSE upper(substr(gift_card_normalize_code(p_code), 3, 3))
  END;
$$;

-- ---------------------------------------------------------------------------
-- 2) get_checkout_gift_cards — redeemable cards for checkout (no full code)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) verify_gift_card_for_checkout — confirm selection without exposing code
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) lookup_gift_card — strip full code for non-super_admin callers
-- ---------------------------------------------------------------------------
...
GRANT EXECUTE ON FUNCTION gift_card_checkout_mask(text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION gift_card_checkout_confirm_chars(text) TO anon, authenticated;
