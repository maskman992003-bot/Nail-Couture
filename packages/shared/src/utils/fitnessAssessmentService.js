...
const DELETE_RPC = 'delete_fitness_assessment';
...
export async function deleteFitnessAssessment(callerPhone, profileId, assessmentId) {
  ...
  try {
    ...
    if (error) {
      ...
      console.error('[fitnessAssessment] delete error:', error);
      return { success: false, error, available: true };
    }

    return { success: true, error: null, available: true };
  } catch (err) {
    console.error('[fitnessAssessment] delete exception:', err);
